...
/**
 * @file NB_ARSS_4_Check.cpp
 *
 * A program to check the non-blocking single-writer, single-reader
 * atomic register implementation in NB_ARSS_4.
 */
...
int main (void)
{
	...
	reg = new NB_ARSS_4<int_fast64_t*>(no_value_indicator, MY_COPY_FNC, MY_GET_FNC, MY_FREE_FNC);
	start = false;
	stop = false;
	error = false;
